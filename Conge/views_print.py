...
def generate_conges_annee_pdf(request):
    ...
    # Récupérer les congés de l'année en cours, triés par nom/prénom
    ...
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="conges_{annee}.pdf"'
    ...
    # Titre
    data_title = [["Liste des Congés de l'Année"]]
    ...
    for conge in conges:
        data.append([
            ...
            Paragraph(conge.typeconge.designation, styleN),
            ...
            Paragraph(
                conge.date_retour_definitif.strftime('%d/%m/%Y') if conge.date_retour_definitif else "Non définie",
                styleN
            )
        ])

    # Largeur des colonnes ajustée
    col_widths = [130, 130, 90, 90, 80]
